...
(function () {
  ...
  const EYE = { w: 210, h: 150, gap: 90, radius: 28 };
  ...
  function resize() {
    ...
    EYE.w = 210 * s; EYE.h = 150 * s; EYE.gap = 90 * s; EYE.radius = 28 * s;
    state.scale = s;
  }
  ...
  function eyePath(shape, w, h, r) {
    ...
    switch (shape) {
      case 'happy': // rounded top, flat-ish bottom arc (smile)
        ...
        ctx.quadraticCurveTo(0, hh * -.45, -hw, hh * .35); ctx.closePath(); break;
      case 'sad':
  ...
  function drawEye(side, pose, hue, cx, cy) {
    ...
    const tint = Math.min(22, Math.abs(pose.hue) * .35);   // emotional hue shifts become a faint tint, never a colour change
    const col = `hsl(${hue} ${tint}% 96%)`, colD = `hsl(${hue} ${tint}% 84%)`, colL = `hsl(${hue} ${tint}% 100%)`;

    ctx.save();
    ctx.translate(cx + ox, cy + oy); ctx.rotate(rot);
    // glow
    ctx.shadowColor = `rgba(255,255,255,${0.35 * glow})`; ctx.shadowBlur = 34 * glow * state.scale;
  ...
  P.headset = (t, w, cx, cy, hue) => {
    ...
    ctx.lineCap = 'round'; ctx.lineWidth = 16 * s; ctx.strokeStyle = '#1a1a1a';
    ctx.beginPath(); ctx.arc(0, 30 * s, span, Math.PI * 1.08, Math.PI * 1.92); ctx.stroke();
    ctx.lineWidth = 6 * s; ctx.strokeStyle = `hsl(0 0% 60%)`; ctx.beginPath(); ctx.arc(0, 30 * s, span, Math.PI * 1.1, Math.PI * 1.9); ctx.stroke();
    for (const side of [-1, 1]) {
      ctx.fillStyle = '#1a1a1a'; rr(side * span - 34 * s, -20 * s, 68 * s, 120 * s, 22 * s); ctx.fill();
      ctx.fillStyle = `hsl(0 0% 55%)`; ctx.shadowColor = `hsl(0 0% 60%)`; ctx.shadowBlur = 20 * s;
      rr(side * span - 20 * s, 8 * s, 40 * s, 64 * s, 14 * s); ctx.fill(); ctx.shadowBlur = 0;
      // pulsing led
      ctx.fillStyle = `hsl(0 0% ${70 + Math.sin(now() * 8) * 25}%)`; ctx.beginPath(); ctx.arc(side * span, 90 * s, 4 * s, 0, 7); ctx.fill();
    }
    ctx.restore();
  };
  P.notes = (t, w, cx, cy, hue) => { const s = state.scale; ctx.save(); ctx.font = `${34 * s}px serif`; ctx.textAlign = 'center'; for (let i = 0; i < 4; i++) { const p = ((t * .35 + i * .27) % 1); const x = cx + (i % 2 ? 1 : -1) * (EYE.gap / 2 + EYE.w + 60 * s + Math.sin(p * 6 + i) * 20 * s); const y = cy + 40 * s - p * 200 * s; ctx.globalAlpha = (1 - p) * .9; ctx.fillStyle = `hsl(0 0% 70%)`; ctx.fillText(['♪', '♫', '♩', '♬'][i], x, y); } ctx.restore(); };
  P.zzz = (t, w, cx, cy, hue) => { const s = state.scale; ctx.save(); ctx.font = `bold ${30 * s}px sans-serif`; ctx.fillStyle = `hsl(0 0% 75%)`; for (let i = 0; i < 3; i++) { const p = ((t * .4 + i * .33) % 1); ctx.globalAlpha = 1 - p; ctx.font = `bold ${(22 + p * 22) * s}px sans-serif`; ctx.fillText('z', cx + EYE.gap / 2 + EYE.w + 30 * s + p * 60 * s, cy - 40 * s - p * 120 * s); } ctx.restore(); };
  P.dots = (t, w, cx, cy, hue) => { const s = state.scale; ctx.save(); for (let i = 0; i < 3; i++) { const p = Math.max(0, Math.sin(t * 4 - i * .8)); ctx.fillStyle = `hsl(0 0% ${55 + p * 30}%)`; ctx.beginPath(); ctx.arc(cx + EYE.gap / 2 + EYE.w + 50 * s + i * 22 * s, cy - EYE.h / 2 - 40 * s - p * 10 * s, 6 * s + p * 3 * s, 0, 7); ctx.fill(); } ctx.restore(); };
  P.bulb = (t, w, cx, cy, hue) => { const s = state.scale; const y = cy - EYE.h / 2 - 90 * s; const g = .6 + Math.sin(t * 6) * .4; ctx.save(); ctx.shadowColor = '#ffffff'; ctx.shadowBlur = 40 * g * s; ctx.fillStyle = `hsl(48 100% ${60 + g * 25}%)`; ctx.beginPath(); ctx.arc(cx, y, 26 * s, 0, 7); ctx.fill(); ctx.shadowBlur = 0; ctx.fillStyle = '#888888'; rr(cx - 12 * s, y + 20 * s, 24 * s, 16 * s, 4 * s); ctx.fill(); ctx.strokeStyle = `hsl(48 100% 70%)`; ctx.lineWidth = 3 * s; for (let i = 0; i < 6; i++) { const a = i * Math.PI / 3 + t; ctx.beginPath(); ctx.moveTo(cx + Math.cos(a) * 40 * s, y + Math.sin(a) * 40 * s); ctx.lineTo(cx + Math.cos(a) * (48 + g * 10) * s, y + Math.sin(a) * (48 + g * 10) * s); ctx.stroke(); } ctx.restore(); };
  P.gear = (t, w, cx, cy, hue) => { const s = state.scale; const x = cx + EYE.gap / 2 + EYE.w + 60 * s, y = cy - EYE.h / 2 - 20 * s; ctx.save(); ctx.translate(x, y); ctx.rotate(t * 2); ctx.fillStyle = `hsl(0 0% 65%)`; for (let i = 0; i < 8; i++) { ctx.rotate(Math.PI / 4); ctx.fillRect(-6 * s, -30 * s, 12 * s, 14 * s); } ctx.beginPath(); ctx.arc(0, 0, 20 * s, 0, 7); ctx.fill(); ctx.fillStyle = '#000'; ctx.beginPath(); ctx.arc(0, 0, 8 * s, 0, 7); ctx.fill(); ctx.restore(); };
  P.pencil = (t, w, cx, cy, hue) => { const s = state.scale; const x = cx + Math.sin(t * 6) * 40 * s, y = cy + EYE.h / 2 + 50 * s + Math.abs(Math.cos(t * 12)) * 6 * s; ctx.save(); ctx.translate(x, y); ctx.rotate(-.7 + Math.sin(t * 6) * .1); ctx.fillStyle = '#e6e6e6'; rr(-8 * s, -60 * s, 16 * s, 70 * s, 3 * s); ctx.fill(); ctx.fillStyle = '#bbbbbb'; rr(-8 * s, -70 * s, 16 * s, 12 * s, 3 * s); ctx.fill(); ctx.fillStyle = '#cccccc'; ctx.beginPath(); ctx.moveTo(-8 * s, 10 * s); ctx.lineTo(8 * s, 10 * s); ctx.lineTo(0, 26 * s); ctx.fill(); ctx.fillStyle = '#333'; ctx.beginPath(); ctx.moveTo(-3 * s, 20 * s); ctx.lineTo(3 * s, 20 * s); ctx.lineTo(0, 26 * s); ctx.fill(); ctx.restore(); };
  P.book = (t, w, cx, cy, hue) => { const s = state.scale; const y = cy + EYE.h / 2 + 60 * s; ctx.save(); ctx.fillStyle = `hsl(0 0% 30%)`; rr(cx - 90 * s, y, 180 * s, 60 * s, 8 * s); ctx.fill(); ctx.fillStyle = '#eeeeee'; rr(cx - 82 * s, y + 6 * s, 80 * s, 48 * s, 4 * s); ctx.fill(); rr(cx + 2 * s, y + 6 * s, 80 * s, 48 * s, 4 * s); ctx.fill(); ctx.strokeStyle = '#bbb'; ctx.lineWidth = 2 * s; for (let i = 0; i < 4; i++) { ctx.beginPath(); ctx.moveTo(cx - 74 * s, y + (16 + i * 9) * s); ctx.lineTo(cx - 14 * s, y + (16 + i * 9) * s); ctx.moveTo(cx + 10 * s, y + (16 + i * 9) * s); ctx.lineTo(cx + 70 * s, y + (16 + i * 9) * s); ctx.stroke(); } ctx.restore(); };
  P.magnifier = (t, w, cx, cy, hue) => { const s = state.scale; const x = cx + EYE.gap / 2 + Math.sin(t * 3) * 30 * s, y = cy + Math.cos(t * 2) * 20 * s; ctx.save(); ctx.strokeStyle = '#eeeeee'; ctx.lineWidth = 8 * s; ctx.beginPath(); ctx.arc(x, y, 40 * s, 0, 7); ctx.stroke(); ctx.fillStyle = 'rgba(180,220,255,.15)'; ctx.fill(); ctx.lineCap = 'round'; ctx.lineWidth = 12 * s; ctx.beginPath(); ctx.moveTo(x + 30 * s, y + 30 * s); ctx.lineTo(x + 65 * s, y + 65 * s); ctx.stroke(); ctx.restore(); };
  P.check = (t, w, cx, cy, hue) => { const s = state.scale; const p = typeof t === 'number' && t <= 1 ? t : 1; const x = cx + EYE.gap / 2 + EYE.w + 70 * s, y = cy - EYE.h / 2 - 30 * s; ctx.save(); ctx.shadowColor = '#ffffff'; ctx.shadowBlur = 20 * s; ctx.strokeStyle = '#ffffff'; ctx.lineWidth = 12 * s; ctx.lineCap = 'round'; ctx.lineJoin = 'round'; ctx.beginPath(); ctx.moveTo(x - 30 * s, y); if (p > .4) ctx.lineTo(x - 30 * s + 25 * s * Math.min(1, (p - .4) / .3), y + 25 * s * Math.min(1, (p - .4) / .3)); if (p > .7) ctx.lineTo(x - 5 * s + 45 * s * Math.min(1, (p - .7) / .3), y + 25 * s - 55 * s * Math.min(1, (p - .7) / .3)); ctx.stroke(); ctx.restore(); };
  P.cross = (t, w, cx, cy, hue) => { const s = state.scale; const x = cx + EYE.gap / 2 + EYE.w + 70 * s, y = cy - EYE.h / 2 - 30 * s; ctx.save(); ctx.shadowColor = '#ffffff'; ctx.shadowBlur = 20 * s; ctx.strokeStyle = '#ffffff'; ctx.lineWidth = 12 * s; ctx.lineCap = 'round'; ctx.beginPath(); ctx.moveTo(x - 25 * s, y - 25 * s); ctx.lineTo(x + 25 * s, y + 25 * s); ctx.moveTo(x + 25 * s, y - 25 * s); ctx.lineTo(x - 25 * s, y + 25 * s); ctx.stroke(); ctx.restore(); };
  P.scale = (t, w, cx, cy, hue) => { const s = state.scale; const y = cy - EYE.h / 2 - 80 * s; const tilt = Math.sin(t * 1.5) * .18; ctx.save(); ctx.translate(cx, y); ctx.strokeStyle = '#dddddd'; ctx.fillStyle = '#dddddd'; ctx.lineWidth = 6 * s; ctx.beginPath(); ctx.moveTo(0, -20 * s); ctx.lineTo(0, 40 * s); ctx.stroke(); ctx.rotate(tilt); ctx.beginPath(); ctx.moveTo(-70 * s, -20 * s); ctx.lineTo(70 * s, -20 * s); ctx.stroke(); for (const sd of [-1, 1]) { ctx.beginPath(); ctx.moveTo(sd * 70 * s, -20 * s); ctx.lineTo(sd * 55 * s, 15 * s); ctx.moveTo(sd * 70 * s, -20 * s); ctx.lineTo(sd * 85 * s, 15 * s); ctx.stroke(); ctx.beginPath(); ctx.arc(sd * 70 * s, 15 * s, 22 * s, 0, Math.PI); ctx.fill(); } ctx.restore(); };
  P.exclaim = (t, w, cx, cy, hue) => { const s = state.scale; const y = cy - EYE.h / 2 - 90 * s + Math.sin(t * 12) * 4 * s; ctx.save(); ctx.font = `bold ${90 * s}px sans-serif`; ctx.textAlign = 'center'; ctx.shadowColor = '#ffffff'; ctx.shadowBlur = 20 * s; ctx.fillStyle = '#ffffff'; ctx.fillText('!', cx + EYE.gap / 2 + EYE.w * .5, y + 30 * s); ctx.restore(); };
  P.question = (t, w, cx, cy, hue) => { const s = state.scale; const y = cy - EYE.h / 2 - 80 * s + Math.sin(t * 3) * 6 * s; ctx.save(); ctx.font = `bold ${80 * s}px sans-serif`; ctx.textAlign = 'center'; ctx.fillStyle = `hsl(0 0% 75%)`; ctx.shadowColor = `hsl(0 0% 60%)`; ctx.shadowBlur = 20 * s; ctx.fillText('?', cx + EYE.gap / 2 + EYE.w * .6, y + 30 * s); ctx.restore(); };
  P.heart = (t, w, cx, cy, hue) => { const s = state.scale; ctx.save(); ctx.font = `${40 * s}px serif`; ctx.textAlign = 'center'; for (let i = 0; i < 5; i++) { const p = ((t * .4 + i * .2) % 1); ctx.globalAlpha = 1 - p; ctx.fillStyle = `hsl(0 0% 65%)`; ctx.fillText('♥', cx + (i - 2) * 70 * s + Math.sin(p * 5 + i) * 15 * s, cy - EYE.h / 2 - 20 * s - p * 160 * s); } ctx.restore(); };
  P.sparks = (t, w, cx, cy, hue) => { const s = state.scale; ctx.save(); for (let i = 0; i < 14; i++) { const p = ((t * .8 + i * .071) % 1); const a = i * 2.39 + t; const R = (EYE.w + EYE.gap / 2) * (1 + p * .6); ctx.globalAlpha = 1 - p; ctx.fillStyle = `hsl(0 0% 70%)`; ctx.beginPath(); ctx.arc(cx + Math.cos(a) * R, cy + Math.sin(a) * R * .7, (2 + (1 - p) * 4) * s, 0, 7); ctx.fill(); } ctx.restore(); };
  P.confetti = (t, w, cx, cy, hue) => { const s = state.scale; ctx.save(); for (let i = 0; i < 40; i++) { const p = ((t * .35 + i * .0251) % 1); const x = cx + ((i * 97) % 700 - 350) * s + Math.sin(p * 8 + i) * 30 * s; const y = cy - 300 * s + p * 640 * s; ctx.globalAlpha = 1 - p * .6; ctx.fillStyle = `hsl(0 0% 60%)`; ctx.save(); ctx.translate(x, y); ctx.rotate(p * 10 + i); ctx.fillRect(-6 * s, -3 * s, 12 * s, 6 * s); ctx.restore(); } ctx.restore(); };
  P.tears = (t, w, cx, cy, hue) => { const s = state.scale; ctx.save(); for (const sd of [-1, 1]) for (let i = 0; i < 3; i++) { const p = ((t * .7 + i * .33) % 1); const x = cx + sd * (EYE.gap / 2 + EYE.w * .5) + sd * 20 * s; ctx.globalAlpha = 1 - p; ctx.fillStyle = '#dddddd'; ctx.beginPath(); ctx.ellipse(x, cy + EYE.h / 2 + p * 120 * s, 6 * s, 10 * s, 0, 0, 7); ctx.fill(); } ctx.restore(); };
  ...
  P.clipboard = (t, w, cx, cy, hue) => { const s = state.scale; const y = cy + EYE.h / 2 + 50 * s; ctx.save(); ctx.fillStyle = '#9a9a9a'; rr(cx - 60 * s, y, 120 * s, 80 * s, 8 * s); ctx.fill(); ctx.fillStyle = '#eeeeee'; rr(cx - 50 * s, y + 12 * s, 100 * s, 60 * s, 4 * s); ctx.fill(); ctx.strokeStyle = `hsl(0 0% 50%)`; ctx.lineWidth = 3 * s; for (let i = 0; i < 4; i++) { const done = (t * .6) % 5 > i + 1; ctx.strokeRect(cx - 42 * s, y + (20 + i * 12) * s, 8 * s, 8 * s); if (done) { ctx.beginPath(); ctx.moveTo(cx - 42 * s, y + (24 + i * 12) * s); ctx.lineTo(cx - 38 * s, y + (28 + i * 12) * s); ctx.lineTo(cx - 32 * s, y + (20 + i * 12) * s); ctx.stroke(); } ctx.beginPath(); ctx.moveTo(cx - 28 * s, y + (24 + i * 12) * s); ctx.lineTo(cx + 40 * s, y + (24 + i * 12) * s); ctx.stroke(); } ctx.restore(); };
  P.wave = (t, w, cx, cy, hue) => { const s = state.scale; ctx.save(); ctx.strokeStyle = `hsl(0 0% 65%)`; ctx.lineWidth = 3 * s; ctx.lineCap = 'round'; for (const sd of [-1, 1]) { const x0 = cx + sd * (EYE.gap / 2 + EYE.w + 40 * s); for (let i = 0; i < 3; i++) { const ph = t * 5 + i; const h = (10 + i * 6) * s * (0.6 + Math.sin(ph) * .4); ctx.globalAlpha = .9 - i * .25; ctx.beginPath(); ctx.moveTo(x0 + sd * i * 14 * s, cy - h); ctx.lineTo(x0 + sd * i * 14 * s, cy + h); ctx.stroke(); } } ctx.restore(); };
  ...
  P.keyboard = (t, w, cx, cy, hue) => { const s = state.scale; const y = cy + EYE.h / 2 + 46 * s; ctx.save(); ctx.fillStyle = '#161616'; rr(cx - 150 * s, y, 300 * s, 54 * s, 10 * s); ctx.fill(); for (let r = 0; r < 3; r++) for (let i = 0; i < 12; i++) { const k = (i * 7 + r * 13 + Math.floor(t * 14)) % 9 === 0; ctx.fillStyle = k ? `hsl(0 0% 65%)` : '#2a2a2a'; if (k) { ctx.shadowColor = `hsl(0 0% 60%)`; ctx.shadowBlur = 10 * s; } else ctx.shadowBlur = 0; rr(cx - 140 * s + i * 24 * s + r * 6 * s, y + 6 * s + r * 15 * s, 18 * s, 11 * s, 3 * s); ctx.fill(); } ctx.restore(); };
  P.progress = (t, w, cx, cy, hue) => { const s = state.scale; const p = typeof t === 'number' ? t : 0; const y = cy + EYE.h / 2 + 50 * s, W2 = 260 * s; ctx.save(); ctx.fillStyle = 'rgba(255,255,255,.08)'; rr(cx - W2 / 2, y, W2, 12 * s, 6 * s); ctx.fill(); ctx.fillStyle = `hsl(0 0% 60%)`; ctx.shadowColor = `hsl(0 0% 60%)`; ctx.shadowBlur = 14 * s; rr(cx - W2 / 2, y, W2 * Math.max(.02, p), 12 * s, 6 * s); ctx.fill(); ctx.shadowBlur = 0; ctx.fillStyle = 'rgba(255,255,255,.6)'; ctx.font = `${11 * s}px ui-monospace,monospace`; ctx.textAlign = 'center'; ctx.fillText(Math.round(p * 100) + '%', cx, y + 30 * s); ctx.restore(); };
  P.laser = (t, w, cx, cy, hue) => { const s = state.scale; const p = typeof t === 'number' ? t : 0; const y = cy - EYE.h / 2 - 20 * s + p * (EYE.h + 40 * s); const span = EYE.gap / 2 + EYE.w + 30 * s; ctx.save(); ctx.strokeStyle = `hsl(0 0% 65%)`; ctx.shadowColor = ctx.strokeStyle; ctx.shadowBlur = 24 * s; ctx.lineWidth = 3 * s; ctx.beginPath(); ctx.moveTo(cx - span, y); ctx.lineTo(cx + span, y); ctx.stroke(); ctx.globalAlpha = .25; ctx.lineWidth = 18 * s; ctx.stroke(); ctx.restore(); };
  ...
  P.cloud = (t, w, cx, cy, hue) => { const s = state.scale; const y = cy - EYE.h / 2 - 110 * s; ctx.save(); ctx.fillStyle = '#666666'; for (const [dx, r] of [[-40, 28], [-10, 38], [30, 30], [55, 22]]) { ctx.beginPath(); ctx.arc(cx + dx * s + Math.sin(t) * 6 * s, y, r * s, 0, 7); ctx.fill(); } ctx.strokeStyle = '#dddddd'; ctx.lineWidth = 3 * s; ctx.lineCap = 'round'; for (let i = 0; i < 9; i++) { const p = ((t * 1.3 + i * .11) % 1); ctx.globalAlpha = 1 - p; const x = cx + (-50 + i * 13) * s + Math.sin(t) * 6 * s; ctx.beginPath(); ctx.moveTo(x, y + 30 * s + p * 150 * s); ctx.lineTo(x - 3 * s, y + 44 * s + p * 150 * s); ctx.stroke(); } ctx.restore(); };
  ...
  P.equalizer = (t, w, cx, cy, hue) => { const s = state.scale; const y = cy + EYE.h / 2 + 60 * s; ctx.save(); for (let i = 0; i < 16; i++) { const h = (6 + abs(Math.sin(t * 7 + i * 1.3) * Math.sin(t * 3 + i)) * 40) * s; ctx.fillStyle = `hsl(0 0% 60%)`; rr(cx - 120 * s + i * 15 * s, y - h, 10 * s, h, 3 * s); ctx.fill(); } ctx.restore(); };

  P.agents = (t, w, cx, cy, hue) => { const s = state.scale; const names = [['D', 200], ['T', 275], ['A', 330], ['Q', 140]]; const y = cy - EYE.h / 2 - 70 * s; ctx.save(); names.forEach(([n, h], i) => { const active = Math.floor(t * 1.1) % 4 === i; const x = cx + (i - 1.5) * 64 * s; const r = (14 + (active ? 4 : 0)) * s; ctx.fillStyle = `hsl(0 0% ${active ? 65 : 30}%)`; if (active) { ctx.shadowColor = `hsl(0 0% 60%)`; ctx.shadowBlur = 22 * s; } ctx.beginPath(); ctx.arc(x, y + (active ? -4 : 0) * s, r, 0, 7); ctx.fill(); ctx.shadowBlur = 0; ctx.fillStyle = active ? '#000' : 'rgba(255,255,255,.5)'; ctx.font = `bold ${13 * s}px sans-serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(n, x, y + (active ? -4 : 0) * s); if (i < 3) { ctx.strokeStyle = 'rgba(255,255,255,.15)'; ctx.lineWidth = 2 * s; ctx.beginPath(); ctx.moveTo(x + r, y); ctx.lineTo(x + 64 * s - r, y); ctx.stroke(); } }); ctx.restore(); };
